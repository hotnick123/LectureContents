...
public class VueMonsterRepository {
    ...
    public void update(Monster monster) throws Exception {
        String query = "update vuemonster set exp = ?, dropItem = ?, dropMoney = ? where monster_no = ?";

        log.info("Vuemonster Repository: " + monster);

        jdbcTemplate.update(query, monster.getExp(), monster.getDropItem(), monster.getDropMoney(), monster.getMonsterNo());
    }
