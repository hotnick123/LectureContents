<template>
    <div class="home">
        <div id="header">
            <router-link :to="{ name: 'Home' }"
                    class="nav-link"
                    active-class="active">
                [Home]
            </router-link>
            <router-link :to="{ name: 'Test' }"
                    class="nav-link"
                    active-class="active">
                [Test]
            </router-link>
             <router-link :to="{ name: 'Todo' }"
                    class="nav-link"
                    active-class="active">
                [Todo]
            </router-link>
                <router-link :to="{ name: 'Homework' }"
                    class="nav-link"
                    active-class="active">
                [Homework]
            </router-link>
             <router-link :to="{ name: 'BoardListPage' }"
                    class="nav-link"
                    active-class="active">
                [BoardListPage]
            </router-link>
            <router-link :to="{ name: 'SignupPage' }"
                    class="nav-link"
                    active-class="active">
                [SignupPage]
            </router-link>
            <router-link :to="{ name: 'GameMemberRegisterPage' }"
                    class="nav-link"
                    active-class="active">
                [GameMemberRegisterPage]
            </router-link>
             <router-link :to="{ name: 'MarketRegisterPage' }"
                    class="nav-link"
                    active-class="active">
                [MarketRegisterPage]
            </router-link>
             <router-link :to="{ name: 'Concave' }"
                    class="nav-link"
                    active-class="active">
                [Concave]
            </router-link>
            <router-link :to="{ name: 'ProductListPage' }"
                    class="nav-link"
                    active-class="active">
               [] ProductListPage]
            </router-link>
             <router-link :to="{ name: 'MonsterListPage' }"
                    class="nav-link"
                    active-class="active">
               [] MonsterListPage]
            </router-link>
             <router-link :to="{ name: 'EventBusTestPage' }"
                    class="nav-link"
                    active-class="active">
               [] EventBusTestPage]
            </router-link>
             <router-link :to="{ name: 'DungeonListPage' }"
                    class="nav-link"
                    active-class="active">
               [] DungeonListPage]
            </router-link>
            <router-link :to="{ name: 'prob89Page' }"
                    class="nav-link"
                    active-class="active">
               [] prob89Page]
            </router-link>
             <router-link :to="{ name: 'springCon' }"
                    class="nav-link"
                    active-class="active">
               [] 삼각형 넓이를 구해보자 ]
            </router-link>
             <router-link :to="{ name: 'ScoreManagePage' }"
                    class="nav-link"
                    active-class="active">
               [] 종합정보시스템 ]
            </router-link>
             <router-link :to="{ name: 'DaumNewsCrawlerPage' }"
                    class="nav-link"
                    active-class="active">
               [] 다음 뉴스 크롤러 ]
            </router-link>
             <router-link :to="{ name: 'UnNamedSlotTestPage' }"
                    class="nav-link"
                    active-class="active">
               [welcome] UnNamedSlot 활용Paage[클릭]]
            </router-link>
              <router-link :to="{ name: 'NamedSlotTestPage' }"
                    class="nav-link"
                    active-class="active">
               [welcome] NamedSlot 활용Page[클릭]]
            </router-link>
            <!-- 기존 slot이 p,div 등의 HTML  태그를 전달하고, 자녀컴포넌트에서 부모컴포넌트를
            사용했음..! 이녀석은 템플릿을 이용하며 부모 컴포넌트로 자녀 컴포넌트의 데이터를 전달할수있음.-->
              <router-link :to="{ name: 'ScopedSlotTestPage' }"
                    class="nav-link"
                    active-class="active">
               [welcome] ScopedSlotTestPage 활용Page[클릭]]
            </router-link>
             <router-link :to="{ name: 'RecentSlotTestPage' }"
                    class="nav-link"
                    active-class="active">
               [welcome] v-slot 활용페이지[클릭]]
            </router-link>
              <router-link :to="{ name: 'MaterialzeTestPage' }"
                    class="nav-link"
                    active-class="active">
               [welcome] MaterialzeTestPage 활용페이지[클릭]]
            </router-link>
             <router-link :to="{ name: 'MaterializeGridTestPage' }"
                    class="nav-link"
                    active-class="active">
               [welcome] MaterialzeTestPage 활용페이지[클릭]]
            </router-link>
              <router-link :to="{ name: 'VuetifyAllInOneTestPage' }"
                    class="nav-link"
                    active-class="active">
               [welcome] VuetifyAllInOneTestPage 활용페이지[클릭]]
            </router-link>

            
        </div>
       
    </div>
</template>

<script>

  export default {
    name: 'Home',
    components: {
    },
  }
</script>