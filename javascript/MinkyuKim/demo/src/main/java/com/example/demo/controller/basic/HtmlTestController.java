...
public class HtmlTestController {
    ...
    // Get, Post 방식인에 우선 일반적인 URL 입력은 Get으로 파악하면 되겠다.
    // "/"는 현재 우리의 ip:port 주소의 Home을 의미한다. 즉 " "자리는 메인페이지를 맵핑하는 자리이다.
    //++ "/index"로 한다면 localhost:7777/index라고 쳐야지 나오게 된다.

    // @Controller가 사용자의 ip:port/ 형식의 요청을 하였을때 동작할 내용을 정의해둔 것.
    @GetMapping("/")
